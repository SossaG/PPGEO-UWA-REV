...
class ViM(nn.Module):
    def __init__(
        ...
        self.mlp_head1 = nn.Sequential(
                ...
                nn.LayerNorm(int(dim / 4)),
                nn.ELU(),
                nn.Linear(int(dim / 4),  int(dim / 8)),
                nn.LayerNorm(int(dim / 8)),
                nn.ELU(),
                nn.Linear(int(dim / 8), 1)).to(device=self.device)
...
def load_latest_data(Search_Folder, Images_All, Speeds_All, Steering_Angles_All):
...
def train_batch(model, criterion, optimizer, scheduler, 
                epoch, temp_search, Images_All, Speeds_All, 
                Steering_Angles_All, train_len, val_len,
                total_epoch_loss,
                total_epoch_val_loss,
                total_epoch_accuracy1,
                total_epoch_accuracy2,
                total_epoch_val_accuracy1,
                total_epoch_val_accuracy2):
    epoch_loss = 0
    epoch_accuracy1 = 0
    epoch_accuracy2 = 0
    total_epoch_loss = 0
    ...
    while (len(Images_All) < 10000) and len(temp_search) != 0:
        current_folder = temp_search.pop()

        load_latest_data(current_folder, Images_All, Speeds_All, Steering_Angles_All)
    ...
    # print((len(temp_search)-2000)/544)
    ...
    # for i in range (0, len(Image_Train)):
        ...
        acc1 = (abs(output1 - label1) < (0.25 / 5.4)).float().sum()
    ...
    with torch.no_grad():
        ...
        # print(train_len)
        return train_len, val_len, total_epoch_accuracy1, total_epoch_accuracy2, total_epoch_loss, \
            total_epoch_val_accuracy1, total_epoch_val_accuracy2, total_epoch_val_loss
        # print(
...
def train_model(model, criterion, optimizer, scheduler, epoch, All_Searchable_Folders, start_time):
    total_epoch_loss = 0
    total_epoch_val_loss = 0
    total_epoch_accuracy1 = 0
    total_epoch_accuracy2 = 0
    ...
    while len(temp_search) != 0:
        batch_len, batch_val_len, tea1, tea2, tel, \
        teva1, teva2, tevl = train_batch(model, criterion, optimizer, scheduler, 
                    epoch, temp_search, Images_All, 
                    Speeds_All, Steering_Angles_All,
                    train_len, 
                    val_len,
                    total_epoch_loss,
                    total_epoch_val_loss,
        ...
        train_len += batch_len
        val_len += batch_val_len
        total_epoch_accuracy1 += tea1
        total_epoch_accuracy2 +=tea2
        total_epoch_loss += tel
        total_epoch_val_accuracy1 += teva1
        total_epoch_val_accuracy2 += teva2
        total_epoch_val_loss += tevl
        print(train_len)
        batch += 1
        for i in range(0,len(Images_All)):
            del Images_All[0]
            del Speeds_All[0]
            del Steering_Angles_All[0]
            # torch.cuda.empty_cache()
        Images_All.clear()
        Speeds_All.clear()
        Steering_Angles_All.clear()
    ...
    if best_loss > total_epoch_loss:
        ...
        save_name = f"vim_shuttle_{model_name}_{epoch+1}_{total_epoch_loss.item():.4f}_{total_epoch_accuracy1.item():.4f}_{total_epoch_accuracy2.item():.4f}.pth"
...
if __name__ == "__main__":
    ...
    model_path_name = "vim_shuttle_reverse_3_0.0013_0.4671_0.7292.pth"
    checkpoint = None
    ...
    for epoch in range(start_epoch, epochs):
        train_model(model, criterion, optimizer, scheduler, epoch, All_Searchable_Folders, start_time)

    end_time = time.strftime("%d-%m-%Y %H:%M:%S", time.localtime())

    print(f'training finished at: {end_time}')

    torch.save(model.state_dict(), f'vim_shuttlebus_{model_name}.pth')
    # writer.flush()
    # writer.close()
