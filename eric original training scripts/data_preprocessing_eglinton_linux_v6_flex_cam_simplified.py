...
class Data_Sorting():
    ...
    def Section_Auto_Sorting(self):
        pass
    def manual_manipulating(self, mode_idx=2):
        mani_mode_list=["Sorting","Modifying","Comparing","Classifying"]
        mani_mode=mani_mode_list[mode_idx]
        self.selected_path_dict = path_dict[dataset_dict[0]]
        self.data_folder_path=self.selected_path_dict["bad_gps_manual_sorting_needed"] 
        ...
        while f_idx < len(sorted_folder_list):
            ...
            while move_file:
                ...
                if mani_mode=="Modifying":
                    ...
                    img_front_ = self.measurements_on_image(img_front_, 5)
                ...
                if key== ord('d'):
                    ...
                    step=20
                elif key in (ord("0"), ord("1"), ord("2"), ord("3")) and mani_mode == "Comparing":
                    state = int(chr(key))
    ...
    def line_on_image(self,image,start_point,end_point):
        ...
        return image
    def measurements_on_image(self,image,splits):
        distance = 80/splits
    ...
    def extract_images_for_sam2(self):
        image_front_folder_path=join(sorted_data_path, "front_images_for_sam2")
        image_rear_folder_path=join(sorted_data_path, "rear_images_for_sam2")
        list_of_folders=["CIL_Dual_Cam_Stage2_B","CIL_Dual_Cam_Stage2_Second_Half"]
        for stage_folder in list_of_folders:
            stage_folder_path=join(sorted_data_path, stage_folder)
            print(stage_folder_path)
            for behavior_folder in os.listdir(stage_folder_path):
                behavior_folder_path=join(stage_folder_path, behavior_folder)
                for bag_folder in os.listdir(behavior_folder_path):
                    bag_folder_path=join(behavior_folder_path, bag_folder)
                    print(bag_folder_path)
                    self.folder_path = bag_folder_path
                    self.sorted_file_list=sorted(os.listdir(self.folder_path), key=lambda x: int(x.split("_")[1]))
                    if len(self.sorted_file_list)>0:
                        print("len: ",len(self.sorted_file_list))
                        for self.idx in range(len(self.sorted_file_list)):

                            linear= self.extract_npy_data()[2]
                            if linear>0.0:
                                front_image= self.extract_npy_data()[0]
                                rear_image= self.extract_npy_data()[1]
                                cv2.imwrite(join(image_front_folder_path, f"{bag_folder}_front_{self.idx}.jpg"), front_image)
                                if rear_image is not None:
                                    cv2.imwrite(join(image_rear_folder_path, f"{bag_folder}_rear_{self.idx}.jpg"), rear_image)
...
if __name__ == '__main__':
    ...
    Data_Sorting().extract_images_for_sam2()