...
from models import SGrayBNPilotNet, SGrayResPilotNet, SGrayMobileNetV3Small, SGrayMobileNetV3Large, SGrayEfficientNetV2S
...
class Trainer:
    ...
    def train_model_from_config(self, cmd_key="cmd_0"):
        ...
        #latest_model_folder = self.get_latest_model_folder()
        latest_model_folder = "/media/erik/Linux_Data/eglinton_data_sorting_dual/saved_models_logs/SGrayBNPilotNet_2025-05-13-23.02.01"
        self.init_wandb()

        if latest_model_folder and self.cfg.training.load_checkpoint:
            checkpoint_path = join(latest_model_folder, f"{self.cfg.model.name}.keras")
            if exists(checkpoint_path):
                print(f"Loading model from checkpoint: {checkpoint_path}")
                self.model = load_model(checkpoint_path)
                if self.cfg.training.checkpoint_lr:
                    print(f"Setting learning rate from {BK.get_value(self.model.optimizer.lr)} to {self.cfg.training.checkpoint_lr}")
                    BK.set_value(self.model.optimizer.lr, self.cfg.training.checkpoint_lr)
            else:
    ...
    def batch_generator(self, subset_type: int, selected_data: list):
        ...
        x, y1, y2 = [], [], []
        offset = -6 #shift labels 6 frames ahead, 0.30 s
        ...
        while True:
            ...
            for ind in indices:
                ...
                dir_name = os.path.dirname(current_data)
                filename = os.path.basename(current_data)
                # For example, if filename is "Img_23388_.npy"
                # and you want to change "23388" to "23379"
                curr_index = filename.split('_')[1]
                new_index = str(int(curr_index) + offset)
                new_filename = filename.replace(curr_index, new_index)
                past_data = current_data.replace(filename, new_filename)
                if not exists(join(self.sorted_data_path, past_data)):
                    continue
                curr_data_array = np.load(join(self.sorted_data_path, current_data), allow_pickle=True)
                past_data_array = np.load(join(self.sorted_data_path, past_data), allow_pickle=True)
                image = curr_data_array[0]
                if len(past_data_array) ==8:
                    speed, steering = past_data_array[2], past_data_array[3]
                elif len(past_data_array) == 10:
                    speed, steering = past_data_array[8], past_data_array[9]
                else:
                    speed, steering = past_data_array[1], past_data_array[2]
